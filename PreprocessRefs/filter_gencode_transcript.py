...
def gencode_codon_check(tfname, pfname, glist, tid2cds, sep=' '):
    """
    exclude :
    1) transcripts that cannot be correctly encoded to peptides
    ...
    5) transcripts which not start with a start codon or frame != 0
    6) transcripts which not stop with a stop codon
    """
    Mcnt = 0
    Ucnt = 0
    tcnt = 0
    ...
    for trec, prec in zip(SeqIO.parse(tfile, "fasta"), SeqIO.parse(pfile, "fasta")):
        ...
        if is_start_codon(tseq[start:start+3]):
		Mcnt += 1
	else:
		glist[tid] = False
	if is_stop_codon(tseq[stop-3:stop]):
		Ucnt += 1
	else:
		glist[tid] = False
	if tid2frame[tid] != 0:
